 ...
 * Date: 2013-10-06
 * modify:konglinghong,Date: 2013-11-04
 * 	 	1/修改 当URL为空时不向后台发送请求
 * 		2/修改 使用searchbar和navsearch查询时 添加请求参数searchString ，值为查询条件的sql语句。
 * 			修改使用searchbar时，切换操作符过程中去除英文关系操作符,
 * 			修改当searchrules中number为true并且操作符为等号时生成的SQL中该项值没有单引号
 * 		3/添加 使用searchbar查询时 输入校验发生错误时使用poshytip作为警告提示(需引入响应的JS和CSS文件)
  * {1:1940; 2：4000,7096 , 3: 4071,4088,4148,4576}
 * 	2013-11-06
 * 		修改searchbar当值为null或是unedfined时，该条件不添加到searchString;
 *
 * modify:konglinghong,Date: 2013-12-16
 *		1/修改 使用searchbar时searchrules的date为true时 添加searchrules参数
 *					{date:true,
*					 dateoption:{
*						database:"mssql",//使用的数据库
*						dtype:"varchar(10)",//规定目标数据类型
*						dcode:"120"// 规定日期/时间的输出格式。
*					}}，将转换该列的查询语句为 convert(dtype,col,dcode)
*		2/添加 在searchbar中日期类使用jquery ui的datepicker时，并且searchrules的date为true，覆盖datepicker的onselect事件，触发searchbar的onEnterSearch事件
*		3/修改 隐藏当使用jquery ui的datepicker时,左下角有明显的datapick DIV元素痕迹
*{1:4055@1216-1; 2：4520@1216-2, 3: 4612@1216-3}
*
*modify:konglinghong,Date: 2014-04-29
*	1/添加 在navbar添加导出按钮,设置"export":true时启用,默认调用执行查询的ACTION方法,参数为exportModel
*	 	c参数内容为当前表格非隐藏列,需要引入JSON2.JS文件
 */
//jsHint options
		...
		$.each(cm,function(i,e){
			...
			if(!e.hidden){
				...
				if(e.align=="center")
					...
					align = 2;
				header.push({
					name:cn[i],width:e.width,align:align,property:e.name
				});
			}
		});
		return header;
...
$.jgrid.extend({
	...
	filterToolbar : function(p){
		p = $.extend({
			...
			afterClear: null,
			searchurl : '',
			stringResult: true,//修改默认值为true
			groupOp: 'AND',
			errorcheck:true,//额外添加
			defaultSearch : "bw",
			searchOperators : true,//修改默认值为true
			operandTitle : "点击选择关系表达式",
			operands : { "eq" :"==", "ne":"!","lt":"<","le":"<=","gt":">","ge":">=","bw":"^","bn":"!^","in":"=","ni":"!=","ew":"|","en":"!@","cn":"~","nc":"!~","nu":"#","nn":"!#"},
			sqlopers : { "eq" :"=", "ne":"<>","lt":"<","le":"<=","gt":">","ge":">=","bw":"LIKE","bn":"NOT LIKE","in":"IN","ni":"NOT IN","ew":"LIKE","en":"NOT LIKE","cn":"LIKE","nc":"NOT LIKE","nu":"IS NULL","nn":"ISNOT NULL"}
		}, $.jgrid.search , p  || {});
		return this.each(function(){
			var $t = this;
			//记录columns,ops
			var columns = this.p.colModel,
				ops = $.jgrid.search.odata;			
			//
			this.getStringForRule = function(rule) {
				var opUF = "",opC="", i, cm, ret, val, dt ,
				numtypes = ['int', 'integer', 'float', 'number', 'currency']; // jqGrid
				for (i = 0; i < ops.length; i++) {
					if (ops[i].oper === rule.op) {
						opUF = p.sqlopers.hasOwnProperty(rule.op) ? p.sqlopers[rule.op] : "";
						opC = ops[i].oper;
						break;
					}
				}
				...
				if(p.errorcheck) { $t.checkData(rule.data, cm); }
				if($.inArray(cm.searchtype, numtypes) !== -1 || opC === 'nn' || opC === 'nu') { ret = rule.field + " " + opUF + " " + val; }
				else {
					if(cm.searchrules && !$.isFunction(cm.searchrules) && cm.searchrules.number ){
						ret = rule.field + " " + opUF + " " + val ;
					//添加类型是DATE时，转换日期函数@1216-1
					}else if(cm.searchrules && !$.isFunction(cm.searchrules) && cm.searchrules.date ){
						if(cm.searchrules.dateoption){
							var o = cm.searchrules.dateoption;
							if(o.database.toLowerCase()==='mssql'){
								ret = "convert("+o.dtype+","+rule.field + ","+o.dcode+")" + opUF + " \'" + val + "\'";
								//此处添加ORACLE
							}else if(o.database.toLowerCase()==='oracle'){
								ret = rule.field + " " + opUF + " to_date(\'" + val + "\',\'"+o.dcode+"\')";
							}
						}else{
							ret = rule.field + " " + opUF + " \'" + val + "\'";
						}
			...
			this.checkData = function(val, colModelItem) {
				var ret = [true,""],$g = $("#" + $.jgrid.jqID($t.id)) || null;
				if($.isFunction(colModelItem.searchrules)) {
					ret = colModelItem.searchrules.call($t, val, colModelItem);
				} else if($.jgrid && $.jgrid.checkValues) {
					try {
						ret = $.jgrid.checkValues.call($t, val, -1, colModelItem.searchrules, undefined);//colModelItem.label);
					} catch (e) {}
				}
				if(ret && ret.length && ret[0] === false) {
					p.error = !ret[0];
					p.errmsg = ret[1].replace(/^_:/,"");
					p.errfield = colModelItem.name;
					
					/**
					 * poshytip 插件不存在
					 */
					if(!$.fn.poshytip){
						var names = $g.getGridParam("colNames");
						var i = $.inArray(colModelItem,$g.getGridParam("colModel"));
						p.cname = names[i--];
					}
				}
				/**
				 ...
				 * 支持(poshytip)
				 */
				if(ret[0]===true && $.fn.poshytip){
					$("#gs_"+colModelItem.name).poshytip("hide");
				}
			};
			...
			var triggerToolbar = function() {
				...
				if(p.stringResult === true || $t.p.datatype === "local") {
					...
					var s = $t.p.postData.filters,
						_reg = /^\"|\"$/g,
						s=$.jgrid.parse(s.replace(_reg,s));
					$.each(['searchField', 'searchOper'], function(i, n){
						if($t.p.postData.hasOwnProperty(n)) { delete $t.p.postData[n];}
					});
					
					$t.p.postData['searchString'] = getStringForGroup(s);//.replace(_reg,"").replace(_reg1,"'");
				} else {
				...
				if(p.error){
					...
					if($.fn.poshytip){
						$('#gs_'+p.errfield).poshytip('update', p.errmsg).poshytip("show");
					}else{
						alert(p.cname+":"+p.errmsg);
					}
					return false;
				}
				var saveurl;
			...
			getStringForGroup = function(group) {
				...
				if (group.groups !== undefined) {
					for (index = 0; index < group.groups.length; index++) {
						...
						try {
							s += $t.getStringForGroup(group.groups[index]);
						} catch (eg) {alert(eg);}
					}
				}
				...
				if (group.rules !== undefined) {
					try{
						for (index = 0; index < group.rules.length; index++) {
							var r = $t.getStringForRule(group.rules[index]);
							if(r){
								if (s.length > 1) {
									s += " " + group.groupOp + " ";
								}
			...
			buildRuleMenu = function( elem, left, top ){
				...
				var str = '<ul id="sopt_menu" class="ui-search-menu" role="menu" tabindex="0" style="font-size:'+fs+';left:'+left+'px;top:'+top+'px;z-index: 99;">',
				...
				for ( i = 0 ; i < options.sopt.length; i++) {
					...
					if(ina !== -1) {
						...
						str += '<li class="ui-menu-item '+selclass+'" role="presentation"><a class="ui-corner-all g-menu-item" tabindex="0" role="menuitem" text="'+p.odata[ina].text+'" value="'+p.odata[ina].oper+'" oper="'+p.operands[p.odata[ina].oper]+'"><table cellspacing="0" cellpadding="0" border="0"><tr><td>'+ p.odata[ina].text+'</td></tr></table></a></li>';
					}
				}
				str += "</ul>";
				$('body').append(str);
				...
				$("#sopt_menu > li > a").hover(
					...
					var v = $(this).attr("value"), txt = $(this).attr("text"),// 取关系操作符中文的值
					oper = $(this).attr("oper");
					$($t).triggerHandler("jqGridToolbarSelectOper", [v, oper, elem]);
					$("#sopt_menu").hide();
					$(elem).text($.trim(txt)).attr("soper",v);
			...
			$.each($t.p.colModel,function(ci){
				...
				thd = $("<div style='position:relative;height:100%;'></div>"),
				...
				if(this.search){
					if(p.searchOperators) {
						...
						for(i = 0;i<$.jgrid.search.odata.length;i++) {
							if($.jgrid.search.odata[i].oper === so) {
								sot = $.trim($.jgrid.search.odata[i].text) || "";
								break;
							}
						}
					...
					case "text":
						...
						//对datapicker的选择事件重新定义，触发查询事件@1216-2
						if($.fn.datepicker && cm.searchrules && cm.searchrules.date){
							$("input",thd).datepicker( "option","onSelect",function(){
								triggerToolbar();
							});
						}
						break;
			 ...
			 */
			if($.fn.poshytip){
				$.each($t.p.colModel,function(ci,ce){
					if(ce.search){
						$("#gs_"+ce.name).poshytip({
							content: '',
							showOn: 'none',
							alignTo: 'target',
							alignX: 'inner-left',
						...
						$("#gs_"+ce.name).bind("blur",function(){
							$(this).poshytip("hide");
						});
					}
				});
...
//				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) + 1)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10))+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
...
"use strict";	
	...
	$.fn.fmatter.money = function(cellval, opts) {
		var op = $.extend({},opts.currency);
		if (opts.colModel.formatoptions == undefined)
			opts.colModel.formatoptions ={decimalSeparator:'.',thousandsSeparator:',',decimalPlace:2};
	...
	$.fn.fmatter.showlink = function(cellval, opts) {
		...
		if(op.target) {target = 'target=' + op.target;}
		if(op.baseLinkUrl.indexOf("javascript")>-1){
...
$.jgrid.extend({
				...
				$("#"+fid+"_search").bind('click', function(){
					...
					if(p.stringResult) {
						...
						if(typeof res==="string") {
							...
							sdata[p.sValue] = fl.jqFilter('toSQLString');//添加searchString;
						}
					} else {
						if(p.multipleSearch) {
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView, pExport) {
		o = $.extend({
			edit: true,
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o["export"]) {
					...
					pExport = pExport || {};
					//添加页面表单
					var _form = document.getElementById("form_export_"+elemids);
					if(!_form){
						$(document.body).append("<form id='form_export_"+elemids+"' style=\"display:none;\" method=\"post\" ><input name=\"searchString\"/><input name=\"exportModel\"/></form>");
					}
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.exporticon+"'></span>"+o.exporttext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.exporttitle || "",id: pExport.id || "export_"+elemids})
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							var dr;
							if($t.p.multiselect) {
							...
							if(!records || records<=0){
								if($.isFunction( top.msgShow)){
									top.msgShow("警告","当前结果数为零,不能导出!","warning");
								}else{
									alert("当前结果数为零,不能导出!");
								}
				           	}else if(records && records>100000){
				           		if($.isFunction( top.msgShow)){
									top.msgShow("导出数据超过100000条,请减少数据再执行导出!","warning");
								}else{
									alert("导出数据超过100000条,请减少数据再执行导出!");
								}
				           	}else{
				               	var postdata = $($t).jqGrid("getGridParam","postData");
				               	var searchString = "";
				               	if(postdata && postdata.searchString){
				               		searchString = postdata.searchString;
				               	}
				               	try{
				               		if(!!JSON2 && $.isFunction(JSON2.stringify)){
						               	$("input","#form_export_"+elemids).each(function(){
											if(this.name=="searchString"){
												this.value = searchString;
											}else{
												this.value = JSON2.stringify($($t).jqGrid("getExportModel"));
											}
										})
										$("#form_export_"+elemids).attr("action",$t.p.url).submit();
					               	}
				               	}catch(e){
				               		alert("导出方法需要引入JSON2.JS文件!");
				               	}
				            }
						}
						return false;
				...
				if(o.add || o.edit || o.del || o.view || o["export"]) {$("tr",navtbl).append(sep);}
				if (o.search) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pSearch = pSearch || {};
				...
				$(ts.rows[i].cells[pos]).bind('click', function() {
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						if(ts.p.subGridOptions.reloadOnExpand === true || ( ts.p.subGridOptions.reloadOnExpand === false && !$(r).hasClass('ui-subgrid') ) ) {
							atd = pos >=1 ? "<td colspan='"+pos+"' style='border-bottom: solid 1px #e7e7e7;'>&#160;</td>":"";
							...
							$(tr).after( "<tr role='row' class='ui-subgrid'>"+atd+"<td class='ui-widget-content subgrid-cell' style='border-bottom: solid 1px #e7e7e7;'></td><td colspan='"+parseInt(ts.p.colNames.length-1-nhc,10)+"' class='ui-widget-content subgrid-data' style='border-right: solid 1px #e7e7e7;border-bottom: solid 1px #e7e7e7;padding: 3px;'><div id="+pID+"_"+_id+" class='tablediv'></div></td></tr>" );
