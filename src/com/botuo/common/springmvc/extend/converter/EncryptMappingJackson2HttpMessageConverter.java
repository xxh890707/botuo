package com.botuo.common.springmvc.extend.converter;
...
import com.botuo.common.encrypt.DESHelper;
 ...
 * @since 3.1.2
 */
public class EncryptMappingJackson2HttpMessageConverter extends AbstractHttpMessageConverter<Object>
	 ...
	 */
	public EncryptMappingJackson2HttpMessageConverter() {
	...
	private void configurePrettyPrint() {
		if (this.prettyPrint != null) {
			objectMapper.configure(SerializationFeature.INDENT_OUTPUT, this.prettyPrint);
		}
	}
	...
	public boolean canRead(Type type, Class<?> contextClass, MediaType mediaType) {
		...
		if (objectMapper.canDeserialize(javaType) && canRead(mediaType)) {
			return true;
		}
	...
	public boolean canWrite(Class<?> clazz, MediaType mediaType) {
		...
		AtomicReference<Throwable> causeRef = new AtomicReference<Throwable>();
		if (objectMapper.canSerialize(clazz) && canWrite(mediaType)) {
			return true;
		}
		Throwable cause = causeRef.get();
		if (cause != null) {
	...
	public Object read(Type type, Class<?> contextClass, HttpInputMessage inputMessage)
		...
		try {
			...
			String decryptStr = desHelper.decrypt(reqStr);
			logger.debug("解密后Json："+decryptStr);
			return objectMapper.readValue(decryptStr, getJavaType(type, contextClass));
		}catch (IOException ex) {
			throw new HttpMessageNotReadableException("Could not read JSON: " + ex.getMessage(), ex);
		}  catch (Exception e) {
			throw new HttpMessageNotReadableException("解密失败: " + e.getMessage(), e);
		}
	}
	...
	protected void writeInternal(Object object, HttpOutputMessage outputMessage)
		...
		try {
			...
			byte[] responseJsonByte = objectMapper.writeValueAsBytes(object);
			DESHelper desHelper = new DESHelper("72fif5zmyrPao8TvrsoabtEXKHiN1PAjjmZU20Vn");
			String responseEncryptStr = desHelper.encrypt(responseJsonByte);
			outputMessage.getBody().write(responseEncryptStr.getBytes());
//			objectMapper.writeValue(jsonGenerator, object);
		}
		catch (JsonProcessingException ex) {
			throw new HttpMessageNotWritableException("Could not write JSON: " + ex.getMessage(), ex);
		} catch (Exception e) {
			throw new HttpMessageNotWritableException("加密失败: " + e.getMessage(), e);
		}
	}
